...
- READY의 과거 거래통계를 백그라운드 query-only SQLite 연결로 준비해 안전 복구와 첫 화면·시작 버튼을 막지 않게 했다. 부팅 단계, 통계 준비상태, 저장 flush의 Parquet·manifest·candle 단계와 최대 이벤트 수신 공백시각을 고급진단에 추가했다.
