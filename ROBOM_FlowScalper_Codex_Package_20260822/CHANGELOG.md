...
- 정상 PAPER 연구가 하루 거래횟수, 하루·주간 손실 또는 연속손실 cooldown 때문에 멈추지 않게 했다. 기본 증거금 배수는 10배이며 설정에서 최대 100배까지 선택한다. 수량은 기존 계획손실·호가깊이 기준으로 정하고 수수료·손익은 실제 체결 명목금액, 증거금은 명목금액을 진입 당시 선택 배수로 나눈 값으로 기록한다. 일반 일시정지·재개 버튼은 제거하되 drawdown·동시포지션·데이터·저장·복구·원장·실행가능 호가 안전대기는 유지한다. 실제 주문·private API·인증·키·지갑은 계속 0이다.
