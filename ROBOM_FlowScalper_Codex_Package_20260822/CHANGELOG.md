...
- LIVE 전략 평가를 미리 준비한 단일 전용 프로세스로 격리해 CPU 계산이 공개시장 이벤트 루프를 막지 않게 하고, Run·설정이 바뀐 오래된 결과는 적용하지 않는다. 사용자 화면의 1초 heartbeat는 유지하면서 상태가 같을 때 무거운 전체 snapshot 재생성은 최대 5초에 한 번으로 줄였다.
