...
- macOS 서비스는 공개시장 연결뿐 아니라 실제 호가 처리 p95 500ms 이하·체결 흐름 p95 1,000ms 이하가 된 뒤에만 준비 완료를 표시한다. 저장 batch를 500건으로 줄이고 관찰구간 안에서 실제로 전진한 flush·WAL 지연만 판정한다. 보강 뒤 120.017초 관찰은 처리·체결 p95 최대 36.839·75.606ms, flush·checkpoint 최대 1.680·1.440초로 PASS했다.
