...
- Wave 154에서 Wave 153을 같은 Run에 안전 설치한 뒤 체크포인트 동시 flush 0은
  ...
  하나라도 발생하면 FAIL로 판정하게 했다. 전략·진입·비용·TP·SL은 바꾸지 않았다.
...
- Wave 152에서 화면의 누적 공동계좌 손익을 오늘 손익처럼 오해할 수 있던 범위를 분리했다.
  ...
  보존한다. 숨겨진 이전 challenger도 family·variant의 한국어 이름으로 표시해 거래·종목 성과의
  `알 수 없는 이전 전략`을 제거했다. backend 1,561건·frontend 121건·공식 Playwright 7 PASS와
  실제 8870 전체 화면 조작을 통과했고 같은 Run·자동진입 허용을 유지했다. 관찰 구간의 자연 적격
  신호는 0이므로 기준을 낮추지 않았고 수익성은 계속 `NOT_PROVEN`이다.
- Wave 151에서 현재 SHADOW 6전략의 TP1·TP2를 고정 R 배수가 아니라
  전략별 실제 가격 구조로 확정하게 바꾸었다. CBR은 최근 10초 충격·눌림,
  VWAP 평균회귀는 최근 2분 이탈 극값·과도이탈 전 가격대 중앙·반대 피벗, 15·30분
  추세 4전략은 완성 15·30분·1시간 피벗·이전 일 고저·돌파 측정폭·ATR
  채널을 사용한다. 추세 전략은 TP1 40% 후 남은 60%를 완성봉 ATR로,
  CBR은 실제 눌림폭으로 추적하며 손절을 넓히지 않는다. 구조가 없거나
  실제 호가·비용 후 순손익비가 부족하면 임의 목표를 만들지 않고 진입을 거부한다.
  손절·1차·2차 익절·runner·확인 구간 근거는 재시작 복구와 원장에 보존하고,
  현재 포지션과 거래 다시보기에서 쉽게 확인할 수 있다. 이전 버전 거래에 없는
  근거는 새로 지어내지 않는다. 기존 포지션은 진입 당시 계획을 유지하며,
  실제 주문·private API·인증은 계속 0이다. 수익성은 `NOT_PROVEN`으로 유지한다.
- Wave 150에서 거래 화면 상단은 현재버전 완료 2회·원장 4행을 받았는데 표는 0건으로
  비던 표시 결함을 수정했다. 경량 화면상태에 남은 이전 `history_scope`가 서버의 현재버전
  묶음 원장을 다시 걸러내던 것이 원인이었다. 완료 표는 이제 `/api/trades`가 같은 응답에 고정한
  현재 전략버전을 기준으로 표시한다. 원장·전략·진입·비용·TP·SL·PAPER 경계는 바꾸지 않았다.
  실제 8870 설치 뒤 BTRUSDT·HYPEUSDT 2기회와 BASE·STRESS 원장 4행이 표 2행으로 표시됨을 확인했다.
- Wave 149에서 15분 계획회전이 서비스 시작 시각을 기준으로 계속 밀려 완성봉 직후에
  정밀 종목이 바뀌던 결함을 수정했다. 기본 회전은 거래소 시각의 매 15분봉 마감 5분 전에
  시작해 새 정밀 종목의 호가 snapshot·depth·trade 준비가 신호 전에 끝나도록 한다. 명시적인
  짧은 진단 회전시간은 그대로 보존한다. 또한 30분 다중추세 재합류가 실제 조건을 통과해도
  화면의 `진입 형태 확인`만 미충족으로 표시되던 상태 대입 누락을 고쳤다. 진입·비용·호가흐름·
  TP·SL 기준과 wide 80·deep 16 상한은 바꾸지 않았다.
  불변 release 설치 뒤 같은 Run을 620.038초 관찰해 계획회전 1·비계획 재연결 0, 공개 event
  +67,936·전략평가 +54,888, gap·resync·drop·저장결함·buffer drop·진입잠금 0을 확인했다.
  관찰 구간의 자연 적격신호와 신규거래는 0이므로 `NOT_OBSERVED`, 수익성은 `NOT_PROVEN`이다.
- Wave 148에서 완료 PAPER 거래 다시보기를 실제 비디오 재생처럼 볼 수 있게 다듬었다.
  데스크톱은 거래 목록과 큰 차트를 나란히 유지하고, 작은 화면은 목록을 기본으로 접어 차트를
  먼저 보여 준다. 거래 선택·재생·신호·진입·종료 이동 시 차트를 화면 중심에 자동으로 맞추고,
  재생·이전·다음·속도 조작부를 차트 안에 두었다. 진입 전·PAPER 진입·실제 종료의 3단계,
  KST 시각·진입 전/후 경과·보유시간·종료 사유를 직관적으로 표시하고, 차트와 하단 고급 Run
  영역이 겹치던 레이아웃 결함을 제거했다. 신호·비용·TP·SL·PAPER 체결 규칙은 바꾸지 않았다.
- Wave 147에서 LIVE 전략 전용 프로세스에 매 평가마다 15분·30분·1시간 완성봉 600개를
  반복 직렬화하던 병목을 제거했다. 완성봉이 바뀔 때만 전체 이력을 보내고, 그 사이에는
  worker의 Run·설정·종목·시간구간별 cache를 재사용한다. worker 재시작으로 cache가 비면
  1회만 전체 이력으로 복구하고 그래도 실패하면 진입 판단을 적용하지 않는다. 직렬화 payload는
  58,799byte에서 2,319byte로 96.056% 줄었다. 변경 전 55초 관찰 두 번의 신규 500ms 초과
  event-loop 지연을 보존하고, 설치 뒤 55초 PASS 세 번에서 신규 500ms 초과 지연·gap·drop·
  비계획 재연결·저장결함 0을 확인했다. 중간 WAL checkpoint 창의 비전략 실패도 삭제하지 않고
  별도 증거로 남겼다. 전략 조건·비용·TP·SL·보수적 PAPER 체결은 바꾸지 않았다.
