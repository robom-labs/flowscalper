...
- `online-snapshot-operator-abort.json`은 지속 writer 환경의 SQLite Online Backup이 완료 진행을 만들지 못해 운영자가 중단한 `ABORTED_OPERATOR`다.
- `online-snapshot-runtime-safety-abort.json`은 LIVE 안전감시가 온라인 snapshot을 중단한 `ABORTED_RUNTIME_SAFETY`다.
