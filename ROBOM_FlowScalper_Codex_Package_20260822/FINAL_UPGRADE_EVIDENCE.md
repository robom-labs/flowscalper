...
## 110. Wave 148 비디오형 차트 중심 거래 다시보기

### 110.1 실제 화면에서 재현한 문제

수정 전 실제 8870 backend에 연결한 source preview에서 완료 `LIVE_PUBLIC` PAPER 거래
54건을 불러왔다. 데스크톱 1280×720에서 최신 HYPEUSDT 거래를 선택했을 때 목록은
300×620px로 보였지만 차트는 화면 y=467에서 시작해 878×567.5px 중 위쪽 일부만
첫 화면에 들어왔다. 재생 조작부는 y=1042.5에 있어 사용자가 차트와 재생 버튼을
함께 보기 어려웠다.

차트를 자동으로 화면 중심에 맞추는 것만으로는 부족했다. 다시보기 grid의 620px 고정
영역 아래에 있던 `저장 Run 직접 확인하기`가 화면 y=624에서 시작하며 y=675.5까지인
차트와 겹쳤다. 이 중첩은 기존 다시보기가 이상하게 보이던 구체적인 레이아웃 결함으로 분류했다.

### 110.2 구현한 사용자 흐름

- 거래 선택 또는 focus session 준비 완료 시 차트를 화면 상단 104∼108px에 자동 배치한다.
  재생, 이전·다음, 신호·진입·종료 이동과 재생 완료도 같은 차트 중심 위치를 유지한다.
- 재생·이전·다음·속도를 차트 하단의 반투명 영상형 조작부로 옮겼다. 따라서 차트를
  보는 상태에서 바로 재생·일시정지·속도 조절을 할 수 있다.
- 상단에 진입 전 확인·PAPER 진입·실제 종료의 3단계를 놓고, 현재 장면 상태,
  KST 시각, `실제 진입 15분 5초 전`, `진입 후 0초`, `1.7시간 보유 후 손절`처럼
  진입 전/후 경과·보유시간·종료 사유를 표시한다.
- 900px 이하에서는 54건 목록을 기본으로 접고 현재 선택 거래 요약과
  `다른 거래 선택` 버튼만 남긴다. 목록을 열면 최대 44dvh, 560px 이하에서는 52dvh·246px
  상한으로 제한하고 거래를 고르면 다시 자동으로 접는다.
- 데스크톱 다시보기 작업 영역을 720px로 분리했다. 수정 후 차트 bottom=675.5,
  고급 Run 영역 top=732로 56.5px 간격이 생겼고 중첩은 false다. 가로 넘침도 false다.

### 110.3 자동검증과 실제 데이터 조작 결과

| 검증 | 상태 | 이번 실행 결과 |
|---|---|---|
| ReplayViewer 집중 Vitest | `PASS` | 8건 |
...
전으로 표시됐다. 실제 진입 이동은 7/41, `2026-09-01 23:30:05 KST`,
진입 83.908·TP1 85.5522·TP2 87.1535·초기 SL 82.8919와 `PAPER 보유 중`을 표시했다.
