...
export function PriceChart({ chart, replay = false }: Props) {
  const geometry = useMemo(() => {
    ...
    values.push(...Object.values(chart.lines).filter((value): value is number => value !== null))
  ...
  const plannedLevels = chart.lines.entry !== null && chart.lines.take_profit !== null && chart.lines.stop !== null
  ...
  return (
    <section className="panel chart-panel" aria-labelledby={replay ? 'replay-chart-title' : 'chart-title'}>
      ...
      <div className="chart-wrap">
        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label={`${chart.symbol} 차트${plannedLevels ? ', 진입 TP SL 선 포함' : ', 진입 계획 없음'}`}>
