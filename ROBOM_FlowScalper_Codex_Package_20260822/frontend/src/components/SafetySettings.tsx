...
export function SafetySettings({ data, onNewRun }: Props) {
  ...
  return (
    <section aria-labelledby="risk-heading">
      ...
      <div className="risk-layout lower-risk-layout"><section className="panel"><h3>현재 적용 중인 안전장치</h3><div className="lock-list">{[...new Set(data.risk.active_locks)].map((lock) => <span key={lock}>{lockLabels[lock] ?? lock}</span>)}</div><p>손실 포지션 물타기, 손실금액을 키우는 방식, 손절선 확대는 구조적으로 금지됩니다.</p></section><section className="panel risk-actions"><h3>모의매매 상태</h3><p role="status" aria-label="자동 진입 제어 상태"><b>{data.operation_status.title_ko}</b></p><p>{data.operation_status.detail_ko}</p><p>새 진입의 일시정지와 재개는 화면 위쪽의 단일 제어를 사용합니다.</p><button type="button" className="danger-button" onClick={onNewRun}>기존 기록 보존 후 새 실험 시작</button><p>새 실험은 과거 기록을 삭제하지 않고 별도로 시작합니다.</p></section></div>
    </section>
  )
}
