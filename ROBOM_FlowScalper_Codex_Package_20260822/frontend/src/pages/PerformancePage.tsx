...
export function PerformancePage({ data, strategies, leagueAccounts, history }: Props) {
  ...
  const summary = [
    ['BASE 독립계좌 합계', formatUsdt(currentTotal, { equity: true })],
    ['BASE 누적 순손익', formatUsdt(currentTotal - startingTotal, { signed: true })],
    ['BASE 총 수수료', formatUsdt(base.reduce((total, account) => total + Number(account.fees_usdt), 0))],
    ['BASE 총 슬리피지', formatUsdt(base.reduce((total, account) => total + Number(account.slippage_usdt), 0))],
    ['BASE 완료 거래', `${base.reduce((total, account) => total + account.trade_count, 0)}건`],
    ['가장 높은 BASE', highest ? `${strategyName(highest.strategy_id)} · ${formatUsdt(highest.current_equity_usdt, { equity: true })}` : '표본 없음'],
    ['가장 낮은 BASE', lowest ? `${strategyName(lowest.strategy_id)} · ${formatUsdt(lowest.current_equity_usdt, { equity: true })}` : '표본 없음'],
  ]
  return (
    <section aria-labelledby="performance-heading">
      <div className="page-heading"><div><p className="section-kicker">전략별 결과</p><h2 id="performance-heading">성과</h2><p className="heading-help">{ordered.length || 8}개 전략의 독립 가상계좌를 같은 기준으로 비교합니다.</p></div><span className="calibrating">표본이 적으면 데이터 모으는 중</span></div>
      <p className="league-warning">BASE {base.length || 8}개 계좌 합계는 하나의 1,000 USDT 공동계좌 수익이 아닙니다.</p>
