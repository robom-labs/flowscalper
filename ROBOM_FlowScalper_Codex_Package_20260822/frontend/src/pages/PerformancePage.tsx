...
export function PerformancePage({ data, strategies, leagueAccounts, history }: Props) {
  ...
  return (
    <section aria-labelledby="performance-heading">
      ...
      {!analyticsReady ? <p className="profile-scope-note" role="status">과거 거래통계를 전략 버전별로 확인하고 있습니다. 준비가 끝나기 전에는 승률·기대값·순위를 표시하지 않습니다.</p> : null}
      ...
      <section className="panel strategy-performance-panel"><div className="panel-title"><div><p className="section-kicker">BASE · STRESS</p><h3>전략별 독립계좌</h3></div><span>총 {leagueAccounts.length}계좌</span></div><div className="table-scroll"><table className="performance-table"><thead><tr><th>전략</th><th>계좌</th><th>이번 Run 현재자산</th><th>현재버전 거래·승률</th><th>기대값</th><th>Profit Factor</th><th>소요시간</th><th>비용</th><th>낙폭</th><th>표본</th></tr></thead><tbody>{ordered.flatMap((strategy) => (['BASE', 'STRESS'] as const).map((profile) => {
        ...
        return <tr key={`${strategy.strategy_id}-${profile}`}><td><strong>{strategy.short_name}</strong><small>{strategy.display_name_ko}</small></td><td>{profile}</td><td>{formatUsdt(account?.current_equity_usdt ?? '1000', { equity: true })}</td><td>{analyticsReady ? <>{report.sample_size}건<small>{report.win_rate === null ? '승률 표본 없음' : `승률 ${formatPercentFraction(report.win_rate)} · ${report.wins}승 ${report.losses}패 ${report.breakevens}보합`}</small></> : <>불러오는 중<small>버전 확인 후 표시</small></>}</td><td>{analyticsReady ? <>{formatUsdt(report.expectancy_usdt)}<small>{formatRatio(report.expectancy_r, ' R')} · {formatRatio(report.expectancy_bps, ' bp')}</small></> : '—'}</td><td>{analyticsReady ? formatRatio(report.profit_factor) : '—'}</td><td>{analyticsReady ? <>{report.median_hold_ms === null ? '표본 없음' : `종료 중앙 ${formatDurationMs(report.median_hold_ms)}`}<small>{report.median_time_to_tp1_ms === null ? 'TP1 표본 없음' : `TP1 중앙 ${formatDurationMs(report.median_time_to_tp1_ms)} · ${report.tp1_sample_size}건`}</small><small>{report.trail_activation_count > 0 ? `추적 활성 ${report.trail_activation_count}건 · 실제 러너 ${report.runner_count}건 · TP1 체결 ${formatPercentFraction(report.tp1_fill_rate)}` : '추적 익절 표본 없음'}</small><small>{report.trail_activation_count > 0 ? `러너 순기여 ${formatUsdt(report.runner_net_contribution_usdt, { signed: true })} · 되돌림 중앙 ${formatUsdt(report.median_peak_giveback_usdt)} / P90 ${formatUsdt(report.p90_peak_giveback_usdt)}` : '추적 후 수익·되돌림 표본 없음'}</small></> : '—'}</td><td>{analyticsReady ? <>수수료 {formatUsdt(report.fees)}<small>슬리피지 {formatUsdt(report.slippage)} · 추적 종료 {formatUsdt(report.trail_trigger_slippage_usdt)}</small></> : '—'}</td><td>{analyticsReady ? formatUsdt(report.maximum_drawdown) : '—'}</td><td>{analyticsReady ? <>{report.sample_status}<small>{report.recommendation} · 과거 버전 {report.excluded_prior_version_samples}건 제외</small></> : '확인 중'}</td></tr>
