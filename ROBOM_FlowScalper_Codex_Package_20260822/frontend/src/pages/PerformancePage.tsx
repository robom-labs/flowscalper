...
export function PerformancePage({ performance, strategies, shadowAccounts, history }: Props) {
  ...
  return (
    <section aria-labelledby="performance-heading">
      <div className="page-heading"><div><p className="section-kicker">RESEARCH HONESTY</p><h2 id="performance-heading">성과분석</h2><p className="heading-help">승률만 보지 않고 비용 이후 기대값·Profit Factor·표본상태를 함께 판단합니다.</p></div><span className="calibrating">{Number(performance.sample_size) < 30 ? '표본 수집 중' : '연구 표본'}</span></div>
      ...
      <section className="panel strategy-performance-panel"><div className="panel-title"><div><p className="section-kicker">STRATEGY BREAKDOWN</p><h3>전략별 BASE·STRESS 비교</h3></div><span>판단은 참고용이며 자동 중지하지 않음</span></div><div className="table-scroll"><table className="performance-table"><thead><tr><th>전략</th><th>비용 프로필</th><th>표본 / 승률</th><th>기대값</th><th>Profit Factor</th><th>비용</th><th>최대 낙폭</th><th>보유 중앙 / p90</th><th>표본상태</th><th>권고</th></tr></thead><tbody>{strategies.flatMap((strategy) => (['BASE', 'STRESS'] as const).map((profile) => {
      ...
      }))}</tbody></table>{strategies.length === 0 ? <p className="empty-copy">전략 성과 레지스트리를 불러오는 중입니다.</p> : null}</div></section>
