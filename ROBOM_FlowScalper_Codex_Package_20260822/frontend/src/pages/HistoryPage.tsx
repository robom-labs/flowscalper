...
export function HistoryPage({ rows, currentRunId, historyScope, onReplay }: Props) {
  ...
  const [accountFilter, setAccountFilter] = useState<AccountFilter>('MAIN')
  ...
  return (
    <section aria-labelledby="history-heading">
      <div className="page-heading">
        ...
        <div className="history-heading-filters"><label className="inline-filter">Run 범위<select value={runFilter} onChange={(event) => setRunFilter(event.target.value as RunFilter)}><option value="CURRENT">이번 Run</option><option value="ALL">전체 Run</option></select></label><label className="inline-filter">계좌 범위<select aria-label="계좌 범위" value={accountFilter} onChange={(event) => setAccountFilter(event.target.value as AccountFilter)}><option value="MAIN">공동 PAPER</option><option value="LEAGUE">전략별 PAPER</option><option value="ALL">모든 PAPER 계좌</option></select></label><label className="inline-filter">비용 조건<select aria-label="비용 조건" value={profileFilter} onChange={(event) => setProfileFilter(event.target.value as ProfileFilter)}><option value="ALL">BASE + STRESS</option><option value="BASE">기본 비용</option><option value="STRESS">강화 비용</option></select></label><label className="inline-filter">전략 버전<select aria-label="전략 버전" value={versionFilter} onChange={(event) => setVersionFilter(event.target.value as VersionFilter)}><option value="CURRENT">현재 버전</option><option value="ALL">과거 버전 포함</option></select></label><label className="inline-filter">기록 구분<select value={filter} onChange={(event) => setFilter(event.target.value as Filter)}><option value="ALL">전체</option><option value="LIVE_PUBLIC">공개시장 모의거래</option><option value="OFFLINE_FIXTURE">샘플 거래</option></select></label></div>
      </div>
      ...
      <div className={selected ? 'history-layout drawer-open' : 'history-layout'}>
        <section className="panel wide-panel table-scroll">
          <table className="history-table"><thead><tr><th>Run / 거래</th><th>종목</th><th>전략 / 계좌</th><th>체결→종료</th><th>종료사유</th><th>총손익</th><th>비용</th><th>순손익</th><th>보유</th><th>보기</th></tr></thead>
            <tbody>{filtered.map((row) => <tr key={`${row.account_scope ?? 'MAIN'}:${row.trade_id}`}><td><strong>{row.trade_id}</strong><small>{row.run_id}<br />{row.sample_type === 'LIVE_PUBLIC' ? '공개시장 PAPER' : '오프라인 DEMO'}</small></td><td>{row.symbol}<small>{row.side}</small></td><td>{row.strategy}<small>{row.profile} · {row.account_scope === 'LEAGUE' ? '전략별 계좌' : '공동계좌'}{row.strategy_version ? ` · ${row.strategy_version}` : ''}</small></td><td>{formatPrice(row.entry)} → {formatPrice(row.exit)}</td><td>{exitReasonLabel(row.exit_reason)}<small>{row.exit_reason}</small></td><td>{formatUsdt(row.gross_pnl)}</td><td>수수료 {formatUsdt(row.fees)}<br />슬리피지 {formatUsdt(row.slippage)}</td><td className={Number(row.net_pnl) >= 0 ? 'positive' : 'negative'}>{formatUsdt(row.net_pnl, { signed: true })}</td><td>{formatDurationMs(row.holding_ms)}</td><td><div className="table-actions"><button type="button" className="table-button" onClick={() => setSelected(row)}>상세</button><button type="button" className="table-button" disabled={row.replay_available === false} title={row.replay_available === false ? '이 Run에는 저장된 공개시장 이벤트가 없습니다.' : undefined} onClick={() => onReplay(row)}>{row.replay_available === false ? '재생 자료 없음' : '재생'}</button></div></td></tr>)}</tbody>
          </table>
          {filtered.length === 0 ? <p className="empty-copy">선택한 범위에는 완료된 PAPER 거래가 없습니다. Run·계좌·비용·버전 범위를 바꿔 확인하세요.</p> : null}
        </section>
