...
export function HistoryPage({ rows, currentRunId, historyScope, onReplay }: Props) {
  ...
  const isCurrentVersion = (row: HistoryRow) => (
    !historyScope?.strategy_version
    || row.strategy_version === historyScope.strategy_version
  )
  ...
  return (
    <section aria-labelledby="history-heading">
      <div className="page-heading">
        <div><p className="section-kicker">TRADE HISTORY</p><h2 id="history-heading">거래 기록</h2><p className="heading-help">공동계좌와 전략별 가상계좌의 종료된 모의거래를 함께 표시합니다.{visiblePriorVersionCount ? ` 선택한 범위의 과거 버전 ${visiblePriorVersionCount}건은 원장에 보관되어 있습니다.` : ''}</p></div>
      ...
      <div className={selected ? 'history-layout drawer-open' : 'history-layout'}>
        <section className="panel wide-panel table-scroll">
          <table className="history-table"><thead><tr><th>Run / 거래</th><th>종목</th><th>전략 / 계좌</th><th>체결→종료</th><th>종료사유</th><th>총손익</th><th>비용</th><th>순손익</th><th>진입→결과</th><th>보기</th></tr></thead>
            <tbody>{filtered.map((row) => <tr key={`${row.account_scope ?? 'MAIN'}:${row.trade_id}`}><td><strong>{row.trade_id}</strong><small>{row.run_id}<br />{row.sample_type === 'LIVE_PUBLIC' ? '공개시장 PAPER' : '오프라인 DEMO'}</small></td><td>{row.symbol}<small>{row.side}</small></td><td>{row.strategy}<small>{row.profile} · {row.account_scope === 'LEAGUE' ? '전략별 계좌' : '공동계좌'}{row.strategy_version ? ` · ${isCurrentVersion(row) ? '현재 전략 버전' : '과거 전략 버전'}` : ''}</small></td><td>{formatPrice(row.entry)} → {formatPrice(row.exit)}</td><td>{exitReasonLabel(row.exit_reason)}<small>{row.exit_reason}</small></td><td>{formatUsdt(row.gross_pnl)}</td><td>수수료 {formatUsdt(row.fees)}<br />슬리피지 {formatUsdt(row.slippage)}</td><td className={Number(row.net_pnl) >= 0 ? 'positive' : 'negative'}>{formatUsdt(row.net_pnl, { signed: true })}</td><td>{formatDurationMs(row.holding_ms)}<small>TP1 {milestoneDuration(row.time_to_tp1_ms, missingMilestone(row, '미도달'))}</small></td><td><div className="table-actions"><button type="button" className="table-button" onClick={() => setSelected(row)}>상세</button><button type="button" className="table-button" disabled={row.replay_available === false} title={row.replay_available === false ? '이 Run에는 저장된 공개시장 이벤트가 없습니다.' : undefined} onClick={() => onReplay(row)}>{row.replay_available === false ? '재생 자료 없음' : '재생'}</button></div></td></tr>)}</tbody>
          </table>
        ...
        {selected ? <aside className="panel trade-drawer" aria-labelledby="trade-detail-title"><div className="panel-title"><h3 id="trade-detail-title">거래 상세</h3><button type="button" className="close-button" aria-label="거래 상세 닫기" onClick={() => setSelected(null)}>닫기</button></div><dl className="detail-list"><div><dt>거래 ID</dt><dd>{selected.trade_id}</dd></div><div><dt>종목 / 방향</dt><dd>{selected.symbol} · {selected.side}</dd></div><div><dt>전략</dt><dd>{selected.strategy}</dd></div><div><dt>수량</dt><dd>{formatQuantity(selected.quantity)}</dd></div><div><dt>실제 진입</dt><dd>{formatPrice(selected.entry)}</dd></div><div><dt>초기 손절</dt><dd>{formatPrice(selected.initial_stop)}</dd></div>{selected.take_profit_1 || selected.take_profit_2 ? <><div><dt>TP1 목표가</dt><dd>{selected.take_profit_1 ? formatPrice(selected.take_profit_1) : '—'}</dd></div><div><dt>TP2 목표가</dt><dd>{selected.take_profit_2 ? formatPrice(selected.take_profit_2) : '—'}</dd></div></> : <div><dt>목표가(과거 기록)</dt><dd>{formatPrice(selected.take_profit)}</dd></div>}<div><dt>실제 종료</dt><dd>{formatPrice(selected.exit)}</dd></div><div><dt>종료 사유</dt><dd>{exitReasonLabel(selected.exit_reason)}</dd></div><div><dt>TP1까지</dt><dd>{milestoneDuration(selected.time_to_tp1_ms, missingMilestone(selected, '미도달'))}</dd></div><div><dt>TP2까지</dt><dd>{milestoneDuration(selected.time_to_tp2_ms, missingMilestone(selected, '미도달'))}</dd></div><div><dt>손절까지</dt><dd>{milestoneDuration(selected.time_to_stop_ms, missingMilestone(selected, selected.exit_reason === 'STOP' ? '기록 확인 필요' : '해당 없음'))}</dd></div><div><dt>실제 종료까지</dt><dd>{formatDurationMs(selected.holding_ms)}</dd></div>{selected.trailing_activation_ts_ms !== null && selected.trailing_activation_ts_ms !== undefined ? <><div><dt>추적 익절 활성화</dt><dd>{formatDurationMs(Math.max(0, selected.trailing_activation_ts_ms - selected.entry_ts_ms))} 뒤</dd></div><div><dt>러너 추적 시작</dt><dd>{selected.runner_started_ts_ms !== null && selected.runner_started_ts_ms !== undefined ? `${formatDurationMs(Math.max(0, selected.runner_started_ts_ms - selected.entry_ts_ms))} 뒤` : '시작 안 됨'}</dd></div><div><dt>최고 미실현 손익</dt><dd>{formatUsdt(selected.peak_unrealized_usdt ?? '0', { signed: true })}</dd></div><div><dt>고점 대비 되돌림</dt><dd>{formatUsdt(selected.giveback_usdt ?? '0')}</dd></div><div><dt>러너 순기여</dt><dd>{formatUsdt(selected.runner_net_pnl_usdt ?? '0', { signed: true })}</dd></div><div><dt>추적 종료 가격차이 비용</dt><dd>{formatUsdt(selected.trail_trigger_slippage_usdt ?? '0')}</dd></div></> : null}<div><dt>수수료 / 슬리피지</dt><dd>{formatUsdt(selected.fees)} / {formatUsdt(selected.slippage)}</dd></div><div><dt>순손익</dt><dd>{formatUsdt(selected.net_pnl, { signed: true })}</dd></div></dl><button type="button" className="primary-button full-width" onClick={() => onReplay(selected)}>이 Run 리플레이 열기</button></aside> : null}
      </div>
    </section>
  )
}
