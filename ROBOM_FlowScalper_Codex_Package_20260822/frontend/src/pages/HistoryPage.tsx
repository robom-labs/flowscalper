  ...
  return (
    <section aria-labelledby="history-heading">
      ...
      <div className={selected ? 'history-layout drawer-open' : 'history-layout'}>
        ...
        {selected ? <aside className="panel trade-drawer" aria-labelledby="trade-detail-title"><div className="panel-title"><h3 id="trade-detail-title">거래 상세</h3><button type="button" className="close-button" aria-label="거래 상세 닫기" onClick={() => setSelected(null)}>닫기</button></div><dl className="detail-list"><div><dt>거래 ID</dt><dd>{selected.trade_id}</dd></div><div><dt>종목 / 방향</dt><dd>{selected.symbol} · {selected.side}</dd></div><div><dt>전략</dt><dd>{selected.strategy}</dd></div><div><dt>수량</dt><dd>{formatQuantity(selected.quantity)}</dd></div><div><dt>실제 진입</dt><dd>{formatPrice(selected.entry)}</dd></div><div><dt>초기 손절</dt><dd>{formatPrice(selected.initial_stop)}</dd></div><div><dt>목표가</dt><dd>{formatPrice(selected.take_profit)}</dd></div><div><dt>실제 종료</dt><dd>{formatPrice(selected.exit)}</dd></div><div><dt>종료 사유</dt><dd>{exitReasonLabel(selected.exit_reason)}</dd></div><div><dt>TP1까지</dt><dd>{milestoneDuration(selected.time_to_tp1_ms, missingMilestone(selected, '미도달'))}</dd></div><div><dt>TP2까지</dt><dd>{milestoneDuration(selected.time_to_tp2_ms, missingMilestone(selected, '미도달'))}</dd></div><div><dt>손절까지</dt><dd>{milestoneDuration(selected.time_to_stop_ms, missingMilestone(selected, selected.exit_reason === 'STOP' ? '기록 확인 필요' : '해당 없음'))}</dd></div><div><dt>실제 종료까지</dt><dd>{formatDurationMs(selected.holding_ms)}</dd></div><div><dt>수수료 / 슬리피지</dt><dd>{formatUsdt(selected.fees)} / {formatUsdt(selected.slippage)}</dd></div><div><dt>순손익</dt><dd>{formatUsdt(selected.net_pnl, { signed: true })}</dd></div></dl><button type="button" className="primary-button full-width" onClick={() => onReplay(selected)}>이 Run 리플레이 열기</button></aside> : null}
      </div>
    </section>
  )
}
