...
export function RiskPage({ data, onPauseToggle, onNewRun }: Props) {
  ...
  const entryButtonLabel = entryAction === 'PAUSE'
    ? '새 진입 잠시 멈추기'
    : entryAction === 'RESUME'
      ? '새 진입 다시 시작하기'
  ...
  return (
    <section aria-labelledby="risk-heading">
      ...
      <div className="risk-layout lower-risk-layout"><section className="panel"><h3>현재 적용 중인 안전장치</h3><div className="lock-list">{[...new Set(data.risk.active_locks)].map((lock) => <span key={lock}>{lockLabels[lock] ?? lock}</span>)}</div><p>손실 포지션 물타기, 마틴게일, 피라미딩, 손절선 확대는 구조적으로 금지됩니다.</p></section><section className="panel risk-actions"><h3>PAPER 제어</h3><button type="button" className="secondary-button" onClick={onPauseToggle} disabled={entryAction !== 'PAUSE' && entryAction !== 'RESUME'}>{entryButtonLabel}</button><p>{data.operation_status.detail_ko}</p><button type="button" className="danger-button" onClick={onNewRun}>기존 기록 보존 후 새 PAPER Run</button><p>새 Run은 과거 기록을 삭제하지 않고 별도 실험으로 시작합니다.</p></section></div>
    </section>
  )
}
