  ...
  return (
    <section aria-labelledby="replay-heading">
      ...
      <section className="panel replay-runbar">
        <label>저장 Run<select value={selectedRun} disabled={loading || previewLoading || timelineLoading || runs.length === 0} onChange={(event) => void changeRun(event.target.value)}>{runs.map((run) => <option value={run.run_id} key={run.run_id}>{run.run_id} · {run.market_event_count === null ? '시장 데이터 저장됨' : `이벤트 ${run.market_event_count.toLocaleString()}건`}</option>)}</select></label>
      ...
      {timeline ? <>
        ...
        <div className="replay-layout">
          ...
          <aside className="panel replay-controls"><h3>동기 재생 제어</h3><div className="control-row"><button type="button" className="primary-button" onClick={() => setPlaying((value) => !value)} disabled={timeline.events.length === 0}>{playing ? '일시정지' : '재생'}</button><button type="button" className="secondary-button" onClick={() => setCursor((value) => Math.min(value + 1, timeline.events.length - 1))} disabled={cursor >= timeline.events.length - 1}>다음 이벤트</button></div><label>속도<select value={speed} onChange={(event) => setSpeed(Number(event.target.value))}>{[0.5, 1, 2, 5, 10, 20, 40, 80].map((value) => <option value={value} key={value}>{value}×</option>)}</select></label><label>이벤트 위치<input type="range" min="0" max={Math.max(0, timeline.events.length - 1)} value={cursor} onChange={(event) => { setCursor(Number(event.target.value)); setPlaying(false) }} /></label><dl><div><dt>현재 이벤트</dt><dd>{timeline.events.length === 0 ? '없음' : `${cursor + 1} / ${timeline.events.length}`}</dd></div><div><dt>저장 전체</dt><dd>{timeline.total_events === null ? '전체 건수 집계 중' : `${timeline.total_events.toLocaleString()}건`}{timeline.truncated ? ` · 화면 ${timeline.events.length.toLocaleString()}건 우선` : ''}</dd></div><div><dt>종류 / 시각</dt><dd>{currentEvent?.event_type ?? '대기'}<br />{currentEvent ? `${formatKstTime(currentEvent.venue_ts_ms)} KST` : '—'}</dd></div><div><dt>bid / ask</dt><dd>{currentQuote ? `${currentQuote.bid} / ${currentQuote.ask}` : '호가 프레임 대기'}</dd></div><div><dt>스프레드</dt><dd>{spreadBps === null ? '—' : `${spreadBps.toFixed(3)} bp`}</dd></div><div><dt>전략 평가</dt><dd>{result ? `${result.strategy_evaluation_count}회` : '실행 전'}</dd></div></dl></aside>
        </div>
        <section className="replay-proof-grid">
          <article className="panel"><span>입력 Checksum</span><b className="checksum">{result ? result.input_checksum ?? '이전 결과 · 입력 전용 checksum 없음' : '백엔드 리플레이를 실행하면 표시됩니다.'}</b></article>
          <article className="panel"><span>종단간 결과</span><b>{result ? `후보 ${result.candidate_plan_count} · 주계좌 ${result.main_trade_count} · 전략별 ${result.shadow_trade_count}` : '전략 검증 전'}</b><small>{result ? `실제 주문 ${result.real_orders_enabled ? '위험' : '0'} · 인증 경로 ${result.auth_required ? '위험' : '0'}` : timeline.preview_only ? '빠른 캔들 미리보기만 표시 중' : '정밀 이벤트를 불러왔으며 전략 검증은 아직 실행하지 않음'}</small></article>
        </section>
        {result ? <details className="panel advanced-details replay-diagnostics"><summary>결정 경로와 최종 상태 보기</summary><div className="diagnostic-columns"><div><h3>결정 경로</h3><ol>{result.decision_path.slice(-20).map((item, index) => <li key={`${item}-${index}`}>{item}</li>)}</ol></div><div><h3>최종 상태</h3><p>{result.final_state}</p><h3>종단간 Checksum</h3><p className="checksum">{result.checksum}</p></div></div></details> : null}
      </> : null}
    </section>
  )
}
