    ...
    <section aria-labelledby="replay-heading">
      ...
      {timeline ? <>
        <div className="replay-layout">
          ...
          <aside className="panel replay-controls"><h3>동기 재생 제어</h3><div className="control-row"><button type="button" className="primary-button" onClick={() => setPlaying((value) => !value)} disabled={timeline.events.length === 0}>{playing ? '일시정지' : '재생'}</button><button type="button" className="secondary-button" onClick={() => setCursor((value) => Math.min(value + 1, timeline.events.length - 1))} disabled={cursor >= timeline.events.length - 1}>다음 이벤트</button></div><label>속도<select value={speed} onChange={(event) => setSpeed(Number(event.target.value))}><option value="0.5">0.5×</option><option value="1">1×</option><option value="4">4×</option><option value="10">10×</option></select></label><label>이벤트 위치<input type="range" min="0" max={Math.max(0, timeline.events.length - 1)} value={cursor} onChange={(event) => { setCursor(Number(event.target.value)); setPlaying(false) }} /></label><dl><div><dt>현재 이벤트</dt><dd>{timeline.events.length === 0 ? '없음' : `${cursor + 1} / ${timeline.events.length}`}</dd></div><div><dt>저장 전체</dt><dd>{timeline.total_events === null ? '2,000건 이상 저장됨' : `${timeline.total_events.toLocaleString()}건`}{timeline.truncated ? ' · 화면 2,000건 제한' : ''}</dd></div><div><dt>종류 / 시각</dt><dd>{currentEvent?.event_type ?? '대기'}<br />{currentEvent ? `${formatKstTime(currentEvent.venue_ts_ms)} KST` : '—'}</dd></div><div><dt>bid / ask</dt><dd>{currentQuote ? `${currentQuote.bid} / ${currentQuote.ask}` : '호가 프레임 대기'}</dd></div><div><dt>스프레드</dt><dd>{spreadBps === null ? '—' : `${spreadBps.toFixed(3)} bp`}</dd></div><div><dt>전략 평가</dt><dd>{result?.strategy_evaluation_count ?? '실행 전'}회</dd></div></dl></aside>
        </div>
