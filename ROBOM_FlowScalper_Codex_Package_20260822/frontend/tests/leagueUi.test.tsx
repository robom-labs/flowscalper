...
test('shows only current non-retired strategies with trust-first ranking evidence', () => {
  ...
  expect(screen.getByText('고정 비율이 아니라 완성봉·실거래 구조에서 1차·2차 가격을 진입 전에 확정')).toBeInTheDocument()
  ...
  expect(screen.getByText(/TP1·TP2는 확정 피벗·이전 일 고저·완성봉 ATR 채널에서 선택/)).toBeInTheDocument()
  fireEvent.click(screen.getAllByRole('button', { name: '전략 상세 정보 닫기' })[0])
