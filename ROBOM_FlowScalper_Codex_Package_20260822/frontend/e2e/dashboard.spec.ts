...
test('시장 중심 PAPER 화면이 데스크톱·태블릿·모바일에서 안정적이다', async ({ page }, testInfo) => {
  ...
  await expect(strategyDialog.getByText('진입 가설', { exact: true })).toBeVisible()
  await expect(strategyDialog.getByText('반증 조건', { exact: true })).toBeVisible()
  await expect(strategyDialog.getByText('근거 약화 종료', { exact: true })).toBeVisible()
  ...
  await expect(strategyDialog.getByText('현재 상태 근거', { exact: true })).toBeVisible()
  ...
  await firstHistoryRow.getByRole('button', { name: '상세' }).click()
  const tradeDetail = page.getByRole('complementary', { name: '거래 상세' })
  await expect(tradeDetail.getByText('TP1 목표가')).toBeVisible()
  await expect(tradeDetail.getByText('TP1 목표가').locator('..').locator('dd')).toHaveText('102')
  await expect(tradeDetail.getByText('TP2 목표가')).toBeVisible()
  await expect(tradeDetail.getByText('TP2 목표가').locator('..').locator('dd')).toHaveText('—')
