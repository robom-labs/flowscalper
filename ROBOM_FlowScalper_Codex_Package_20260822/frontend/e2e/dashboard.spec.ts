...
test('시장 중심 PAPER 화면이 데스크톱·태블릿·모바일에서 안정적이다', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('8개 감시 · 검증 중지 2개 · 문제 0개 · 실제 주문 0')).toBeVisible()
  await expect(page.getByText('준비 중')).toHaveCount(8)
  await expect(page.locator('.strategy-monitor.off')).toHaveCount(2)
