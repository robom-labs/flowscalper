...
test('시장 중심 PAPER 화면이 데스크톱·태블릿·모바일에서 안정적이다', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.strategy-compact-table tbody tr')).toHaveCount(6)
  await expect(page.getByText('6/6 전략 켜짐 · 실제 주문 0')).toBeVisible()
  await expect(page.locator('.strategy-inline-modes button[aria-pressed="true"]')).toHaveCount(6)
  await expect(page.locator('.strategy-inline-directions button[aria-pressed="true"]')).toHaveCount(12)
