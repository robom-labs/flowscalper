...
위 28회 flush 표본 이후 같은 `run-2b0119b86432`를 159,663 이벤트·79회 flush까지 계속 관찰하자 최장 flush가 다시 15.783초로 증가했다. 최장 Parquet 구간은 0.252초였지만 통합 원장 커밋이 15.520초였고, 최대 이벤트 수신 공백 11.823초와 임계 지연 사건 6회·최장 90.400초가 기록됐다. 비계획 재연결·sequence gap·drop·저장 fault는 0이었다.

따라서 앞의 56,260 이벤트 결과는 초기 표본으로는 사실이지만 장시간 개선 완료 증거로 사용하지 않는다. 원자 커밋은 두 번의 FULL 커밋을 한 번으로 줄이고 원자성을 보장했지만, 약 1,000 WAL page마다 COMMIT 호출 스레드에서 실행되던 SQLite 기본 자동 checkpoint는 제거하지 못했다. 후속 원인과 해결은 ADR-030에서 다룬다.
