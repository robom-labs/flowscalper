...
11. 이 보호는 전략 임계값, 비용, fill, TP1, TP2, SL, 위험예산, Governor와 30표본·70%·OOS
    gate를 바꾸지 않는다.
12. 전체 연구 자식은 16개 이벤트마다 공통 replay CPU 예산에 협조하고, LIVE 안전 실행기는
    기본 CPU 목표 비율을 0.25로 전달한다. 요청값과 결과값이 다르면 결과를 게시하지 않는다.
    CPU 배정값은 전략·데이터·비용 시험의 정체성에는 포함하지 않아 배정값만 바꾼 중복 시험을
    새 전략 변형으로 세지 않는다.
...
2026-08-29 실제 TP1 전체 gate 실행은 808.772초 동안 LIVE 이벤트 52,915건이 전진했지만,
500ms 초과 event-loop counter가 1회 증가해 `ABORTED_RUNTIME_SAFETY`로 안전 중단됐다. 부분
성과는 제거했고 순위에 넣지 않았다. 원인은 연구 자식에 협조 CPU 예산이 연결되지 않은
실행경로 차이로 좁혔으며, 16-event checkpoint·기본 0.25 목표·명령 및 결과 일치 검증을
회귀계약에 추가한 뒤 동일 동결 입력의 불완료 시험만 재시도한다.
