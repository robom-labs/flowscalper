...
- 현재 완전복사된 Train 논리구간 2,502파일이 manifest size와 SHA-256을 모두 통과한다.
...
위생과 30개 누적 회귀계약도 통과했다. RAM의 Train 2,502파일은 0.647초에 size·SHA-256
검사를 모두 통과했다.
...
fault·buffer drop 0으로 PASS했다. 적격신호와 신규 거래는 0이므로 수익성은 `NOT_PROVEN`,
실자금은 `NOT_READY`다. Validation 복제와 새 사전등록 뒤의 전체 100후보 screening은 아직
`NOT_RUN`이다.
