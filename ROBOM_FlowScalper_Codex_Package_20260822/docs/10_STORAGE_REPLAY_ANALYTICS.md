...
- The LaunchAgent calls a bootstrap on the outer external drive. It remounts the APFS sparsebundle when necessary, rotates external logs and then `exec`s the external immutable runner. A missing outer drive or sparsebundle is an explicit unavailable state, not a successful localhost service.
