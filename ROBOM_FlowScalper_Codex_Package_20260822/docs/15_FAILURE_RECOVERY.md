...
- The storage guard checks both the active SQLite ledger volume and public-market archive volume at most once per second. Below 2GiB or 5% free by default on either volume, LIVE entries remain locked while existing PAPER position management stays independent of the browser. Diagnostics report archive and ledger capacity separately and prefix the lock reason with `ARCHIVE_` or `LEDGER_` when they differ.
