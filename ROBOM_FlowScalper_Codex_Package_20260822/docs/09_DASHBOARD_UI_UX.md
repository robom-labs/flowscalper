...
- current system safety locks;
- create new Run dialog;
- continuous PAPER entry status and selected margin leverage;
- symbol denylist;
...
- 정상 실행은 별도 일시정지·재개 버튼 없이 `자동 진입 · 항상 허용`을 표시한다.
- 데이터·저장·복구 안전대기는 `자동 진입 · 안전 대기`로 표시하고 조건 정상화 뒤 자동 복구한다.
- 설정 화면은 기본 10배와 1~100배 허용 선택지를 보여주며, 실제 명목금액 기준 수수료와 위험기반 수량의 차이를 쉬운 문장으로 설명한다.
...
- `거래 > 다시보기`는 완료된 공개시장 PAPER 거래 목록과 선택 거래의 큰 차트를 기본으로 연다.
  데스크톱은 목록·차트 분할, 태블릿·모바일은 차트 중심 세로 배치이며 목록 선택 때문에 차트 폭이나
  높이가 흔들리지 않아야 한다.
- 첫 화면은 신호 이전 완성 캔들이다. 재생 시각이 진행될 때 신호, 실제 진입, TP1, TP2, 실제 종료가
  순서대로 나타나고 초기 SL·TP 계획선은 신호 뒤에만 보인다.
- 사용자는 처음부터, 이전·다음, 재생·일시정지, 속도와 신호·진입·TP1·TP2·종료 시점 이동을 쓸 수
  있다. 재생 끝에서 다시 재생하면 처음부터 시작한다.
- 차트 아래에는 쉬운 한국어로 진입 근거, 전략의 필수 시간구간, 정확한 KST 신호·진입·종료 시각,
  보유시간, 종료 이유와 비용후 결과를 표시한다. 원시 코드·수수료·버전은 접힌 고급 진단에 둔다.
- 과거 거래 설명이 현재 Registry에서 온 참고 정보라면 거래 당시 저장 버전과의 일치 여부를 함께
  표시한다. 알 수 없는 정보를 현재 사실처럼 추정하지 않는다.
- `STOP_HIT`은 실제 종료점에 표시하고 초기 SL은 계획선으로 분리한다. 미래 시점의 완성되지 않은
  캔들·TP·종료 표시는 현재 재생 커서보다 먼저 보여 주지 않는다.

## 9.20 오늘 활동과 누적 성과의 분리

- `오늘 공동손익`은 현재 Run에 속한 MAIN 공동계좌 완료 거래 중 `Asia/Seoul` 종료일이 오늘인
  거래만 합산한다. 현재 전략 버전으로 한정하지 않아 배포 전후의 같은 Run 공동계좌 결과를
  임의로 숨기지 않는다.
- `오늘 전략 모의결과`는 현재 Run·현재 전략 버전의 독립 전략계좌 결과를 보여준다. 같은 진입기회의
  BASE·STRESS 비용 결과는 고유기회 1회로 묶고, 원장 행 수는 내부 진단에서 별도로 보존한다.
- 공동계좌 손익과 전략별 독립 모의결과는 서로 다른 계좌·표본이므로 한 숫자로 합치지 않는다.
  화면은 `오늘`과 `누적`, `고유기회`와 `원장 행`의 범위를 문구로 명시한다.
- 상단 현재자산, 공동계좌 누적 순손익, 누적 수수료와 공동계좌 거래 수는 누적값이다. 오늘 값처럼
  표시하거나 전략별 BASE·STRESS 결과를 공동계좌 거래 수에 더하지 않는다.
- 오늘 완료 거래가 0건이어도 공개시장 이벤트와 전략 평가가 전진하고 새 PAPER 진입이 허용돼 있으면
  `작동 중`을 유지한다. 이때 화면은 조건 대기 사유를 보여주며 거래를 만들기 위해 기준을 낮추지 않는다.
- 보존된 이전 전략의 표시명은 registry의 한국어 family·variant 이름을 우선한다. 유효한 표시명이
  있으면 raw ID나 `알 수 없는 이전 전략`을 기본 화면에 노출하지 않는다.
