...
| 101 | IN_PROGRESS_GITHUB_REVERIFY | WORKTREE | GitHub Linux에서 현재 RSS 219.031MiB가 ru_maxrss peak 218.754MiB보다 크 교차 계측으로 Actions `33143465472`가 backend 647건 중 1건 FAIL했다. peak를 현재 RSS 하한으로 보정하고 출처를 명시했다. 전용 2건×10회와 GitHub과 동일 전체 명령의 backend 648·frontend 72·build·PAPER safety·security·hygiene는 로컬 PASS다. | 수정 commit의 GitHub main·Actions는 `NOT_RUN`이다. | 불변 release 설치·실제 서비스 복구·GitHub main·Actions를 재검증한다. |
