...
| Upgrade 47 | IN_PROGRESS | 현재 메모리로 표시하던 `ru_maxrss` 최고치와 운영체제 현재 RSS의 불일치를 재현했다. 플랫폼별 현재 RSS와 별도 최고 RSS, soak 분리 지표, 한국어 고급진단 문구와 회귀 테스트를 구현했다. | 재시작 실제 서비스·브라우저·전체 회귀·6시간 경계·GitHub Actions 검증 진행 중. 24h·Release ZIP NOT_RUN | 기존 Run의 6시간 경계를 보존 관찰한 뒤 평평한 상태에서 재시작·실제 화면·전체 회귀·GitHub 동기화 |
...
| 47 | IN_PROGRESS | pending | 현재 RSS·최고 RSS 분리 코드와 backend·frontend 표적 회귀 PASS. 실제 재시작·브라우저·전체 회귀·GitHub 검증은 아직 진행 중이다. | 6h 경계 대기, 24h·Release ZIP NOT_RUN | 실제 서비스 교체 전 기존 Run 6h 증거 보존 후 최종 검증 |
